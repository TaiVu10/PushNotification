...
});
{
  ...
  registerRoute(({ url }) => url.pathname.startsWith(`${scopePath}VAADIN/__vite_ping`), networkOnly);
  registerRoute(({ url }) => url.pathname.startsWith(`${scopePath}VAADIN/`), networkFirst);
  {
    registerRoute(({ request, url }) => request.mode === "navigate" && !isManifestEntryURL(url), async ({ event }) => {
      return networkFirst.handle({ request: new Request(offlinePath), event }).then(rewriteBaseHref);
    });
  }
}
